...
const dashboard = http.createServer(function (_req, res) {
        ...
        res.write("CYBER-CAT IS READY TO GO!!");
        res.end();
});

dashboard.listen(process.env.port || 0);

logger("The server is ready to go", "[ START ]");
...
function startBot(message) {
  ...
  child.on("close", (codeExit) => {
    if (codeExit != 0 || global.countRestart && global.countRestart < 5) {
      startBot("The bot is restarting...");
  ...
  child.on("error", function(error) {
    logger("Error! An error occurred. Please try again later: " + JSON.stringify(error), "[ Starting ]");
  });
};
////////////////////////////////////////////////
...
axios.get("https://raw.githubusercontent.com/abdullarahaman/CatBotData/main/package.json").then((res) => {
