...
async function noprefix(arg) {
    ...
    for (const el of arr) {
        ...
        if (regEx_tiktok.test(el)) {
            ...
            out({
                body: `====『 𝐓𝐈𝐊𝐓𝐎𝐊 𝐕𝐈𝐃𝐄𝐎 』====\n━━━━━━━━━━━━━━━━━━━━\n🕵️ NAME: ${data.author.nickname}\n🆔 𝐔𝐈𝐃: ${data.author.unique_id}\n🌐 Contact: ${data.region}\n📌 Tag: ${data.title}\n❤️ react: ${data.digg_count}\n💬 Comments: ${data.comment_count}\n♻️ Share: ${data.share_count}\n⬇️ Download: ${data.download_count}\n⏳ Duration: ${data.duration} sec\n━━━━━━━━━━━━━━━━━━━━\n✿ Creator "❤️"  𓊈𝙎𝘼𝙄𝙁𝙐𝙇-𝘽𝙊𝙏𓊉
...
╰──────•◈•──────╯ ✿`, attachment: await streamURL(data.play, 'mp4')
        ...
        /* TỰ ĐỘNG TẢI VIDEO FACEBOOK */
            ...
            attachment: await streamURL((fdl = (await axios.get(`https://api.naht-fus.repl.co/facebook/download?url=${el}`)).data.data, fdl.videos.hd), 'mp4'), body: `===『 𝐅𝐀𝐂𝐄𝐁𝐎𝐎𝐊 𝐕𝐈𝐃𝐄𝐎 』===\n━━━━━━━━━━━━━━━━━━`
        }, '', (err, dataMsg) => global.client.handleReaction.push({
        ...
        if (regEx_instagram.test(el))out({
            attachment: await streamURL((idl = (await axios.get(`https://api.naht-fus.repl.co/Instagram/videodl?url=${el}`)).data.video_versions.url, idl[((irx = /\/p\//.test(el))?'display': 'video')+'_url']), irx?'jpg': 'mp4'), body: !irx?'→ 𝗧𝗵𝗮̉ 𝗰𝗮̉𝗺 𝘅𝘂́𝗰 🐧 đ𝗲̂̉ 𝘁𝗮̉𝗶 𝗻𝗵𝗮̣𝗰.':''
